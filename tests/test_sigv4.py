...
from s3proxy.s3client import CLOCK_SKEW_TOLERANCE, ParsedRequest, S3Credentials, SigV4Verifier
...
class TestSigningKeyDerivation:
    ...
    def test_signing_key_format(self, verifier):
        ...
        signing_key = verifier._get_signing_key(secret_key, date_stamp, region, service)

        # Signing key should be 32 bytes (SHA256 output)
        assert len(signing_key) == 32
        assert isinstance(signing_key, bytes)

    def test_signing_key_deterministic(self, verifier):
        ...
        key1 = verifier._get_signing_key(secret_key, date_stamp, region, service)
        key2 = verifier._get_signing_key(secret_key, date_stamp, region, service)

        assert key1 == key2

    def test_different_dates_different_keys(self, verifier):
        """Test different dates produce different signing keys."""
        secret_key = "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"

        key1 = verifier._get_signing_key(secret_key, "20240115", "us-east-1", "s3")
        key2 = verifier._get_signing_key(secret_key, "20240116", "us-east-1", "s3")
