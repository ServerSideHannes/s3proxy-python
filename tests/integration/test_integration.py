...
class TestMultipartEncryptionFlow:
    ...
    async def test_list_multipart_uploads(self, mock_s3):
        ...
        await mock_s3.create_multipart_upload(bucket, "file1.bin")
        await mock_s3.create_multipart_upload(bucket, "file2.bin")
        await mock_s3.create_multipart_upload(bucket, "subdir/file3.bin")
...
class TestListObjects:
    ...
    @pytest.mark.asyncio
    async def test_list_objects_with_delimiter(self, mock_s3):
        """Test V2 list with delimiter for grouping (CommonPrefixes)."""
        await mock_s3.create_bucket("test-bucket")
        ...
        await mock_s3.put_object("test-bucket", "dir2/subdir/file4.txt", b"data")

        resp = await mock_s3.list_objects_v2("test-bucket", delimiter="/")
    ...
    @pytest.mark.asyncio
    async def test_list_objects_with_prefix_and_delimiter(self, mock_s3):
        """Test V2 list with both prefix and delimiter."""
        await mock_s3.create_bucket("test-bucket")
        await mock_s3.put_object("test-bucket", "backup/2024/01/file1.txt", b"data")
        await mock_s3.put_object("test-bucket", "backup/2024/02/file2.txt", b"data")
        await mock_s3.put_object("test-bucket", "backup/2024/03/file3.txt", b"data")
        await mock_s3.put_object("test-bucket", "backup/2025/01/file4.txt", b"data")

        # List with prefix "backup/" and delimiter "/" should return year prefixes
        resp = await mock_s3.list_objects_v2("test-bucket", prefix="backup/", delimiter="/")
        assert len(resp["Contents"]) == 0  # No files directly under backup/
        common_prefixes = [cp["Prefix"] for cp in resp.get("CommonPrefixes", [])]
        assert "backup/2024/" in common_prefixes
        assert "backup/2025/" in common_prefixes
        assert len(common_prefixes) == 2
...
class TestEncryptedCopyObject:
    ...
    async def test_copy_encrypted_object(self, mock_s3, settings):
        ...
        await mock_s3.put_object(
            "test-bucket", "source.txt", encrypted.ciphertext, metadata=metadata
        )
...
class TestInternalPrefixFiltering:
    ...
    async def test_internal_prefix_hidden(self, mock_s3):
        ...
        from s3proxy.state import INTERNAL_PREFIX
    ...
    async def test_legacy_suffix_hidden(self, mock_s3):
        ...
        from s3proxy.state import META_SUFFIX_LEGACY
...
class TestUploadPartCopy:
    ...
    async def test_upload_part_copy_basic(self, mock_s3):
        ...
        copy_resp = await mock_s3.upload_part_copy(
            "test-bucket", "dest.txt", upload_id, 1, "test-bucket/source.txt"
        )
    ...
    async def test_upload_part_copy_nonexistent_source(self, mock_s3):
        ...
        with pytest.raises(Exception) as exc_info:
            await mock_s3.upload_part_copy(
                "test-bucket", "dest.txt", upload_id, 1, "test-bucket/nonexistent.txt"
            )
        assert "NoSuchKey" in str(exc_info.value)
