...
# Assert the dashboard reports a MULTIPART object as encrypted (issue #47
# #6). Multipart objects keep their wrapped DEK in a sidecar, not an on-object
# tag, so the dashboard API must consult the sidecar. The byte-level verify_encryption
# above can't catch a mislabel in the dashboard — this closes that gap.
#
#   verify_dashboard_encryption <bucket> <namespace>
# Picks a real multipart backup object (ETag ending in "-<n>") and asserts the
# dashboard object-detail API returns "encrypted": true for it.
verify_dashboard_encryption() {
    local BUCKET="$1"
    local NAMESPACE="${2:-default}"

    echo "=== Dashboard Encryption-Status Check (multipart) ==="
    ...
    local KEY
    KEY=$(kubectl run dashboard-enc-find --namespace minio --rm -i --restart=Never \
    ...
    # Query the dashboard object-detail API from inside an s3proxy pod (which has
    # Python + reaches its own dashboard API on localhost). The {key:path} route
...
key = '''$KEY'''
url = 'http://localhost:4433/dashboard/api/objects/$BUCKET/' + key
req = urllib.request.Request(url)
req.add_header('Authorization', 'Basic ' + base64.b64encode(b'admin:admin').decode())
print(urllib.request.urlopen(req, timeout=15).read().decode())
" 2>&1)
    echo "Dashboard response: $RESP"

    if echo "$RESP" | grep -q '"encrypted": *true'; then
        echo "✓ Dashboard API reports multipart object encrypted"
        return 0
    fi
    echo "✗ Dashboard API reported multipart object as NOT encrypted"
    return 1
}
